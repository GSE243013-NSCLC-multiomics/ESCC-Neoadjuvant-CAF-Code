...
cat("============================================================\n")
cat("STEP 18D2: COPYKAT P16T\n")
cat("============================================================\n\n")

OUTDIR <- "06_tables/cross_dataset/malignant_epithelial/GSE160269_copykat_validation/P16T"
...
cat("1. Loading P16T raw counts...\n")

raw_counts <- readRDS(file.path(BASEDIR, "P16T_raw_counts.rds"))
annot <- fread(file.path(BASEDIR, "P16T_cell_annotations.csv"))
...
# Convert to data.frame (CopyKAT requires data.frame, not sparse matrix)
cat("   Converting sparse matrix to data.frame...\n")
raw_counts_df <- as.data.frame(as.matrix(raw_counts))
cat("   Data.frame created:", nrow(raw_counts_df), "genes x", ncol(raw_counts_df), "cells\n\n")

# CopyKAT with default parameters
copykat_result <- tryCatch({
  copykat(
    rawmat = raw_counts_df,
    id.type = "S",
    cell.line = "no",
    ngene.chr = 5,
    LOW.DR = 0.05,
  ...
  stop("Step 18D2 stopped during CopyKAT.")
})
...
saveRDS(copykat_result, file.path(OUTDIR, "P16T_copykat_object.rds"))
cat("   Saved: P16T_copykat_object.rds\n")

# Save predictions
fwrite(as.data.table(predictions, keep.rownames = "cell_barcode"), 
       file.path(OUTDIR, "P16T_copykat_predictions.csv"))
cat("   Saved: P16T_copykat_predictions.csv\n")

# Save CNA matrix if available
if (!is.null(copykat_result$CNV.matrix)) {
  saveRDS(copykat_result$CNV.matrix, file.path(OUTDIR, "P16T_copykat_CNA_matrix.rds"))
  cat("   Saved: P16T_copykat_CNA_matrix.rds\n")
}
...
fwrite(summary_dt, file.path(OUTDIR, "P16T_copykat_summary.csv"))
cat("   Saved: P16T_copykat_summary.csv\n")
...
cat("\n============================================================\n")
cat("STEP 18D2 COPYKAT P16T COMPLETE\n")
cat("============================================================\n\n")
