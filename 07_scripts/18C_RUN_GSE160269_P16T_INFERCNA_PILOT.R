...
# STEP 18C
#
# Run InferCNA on GSE160269 P16T pilot
#
# Tumor:  P16T epithelial cells = 2341
...
cat("\n============================================================\n")
cat("STEP 18C: GSE160269 P16T INFERCNA PILOT\n")
cat("============================================================\n\n")
...
indir <- "06_tables/cross_dataset/malignant_epithelial/GSE160269_P16T_pilot"

outdir <- file.path(indir, "18C_results")

figdir <- "05_figures/GSE160269/malignant_epithelial/P16T_pilot"
...
cpm_file <- file.path(indir, "GSE160269_P16T_infercna_CPM_hg38.rds")
refs_file <- file.path(indir, "GSE160269_P16T_infercna_refCells.rds")
annotation_file <- file.path(indir, "GSE160269_P16T_infercna_cell_annotation.csv")
gene_file <- file.path(indir, "GSE160269_P16T_infercna_gene_table.csv")
...
print(params)
fwrite(params, file.path(outdir, "GSE160269_P16T_18C_parameters.csv"), bom = TRUE)
...
cat("\n============================================================\n")
cat("8. RUNNING INFERCNA — P16T PILOT\n")
cat("============================================================\n\n")

cat("This is the first GSE160269 CNV run.\n")
...
cna <- tryCatch(
  ...
  error = function(e) {
    ...
    stop("Step 18C stopped during InferCNA.")
  }
)
...
all_cna_file <- file.path(outdir, "GSE160269_P16T_infercna_CNA_all_cells.rds")
tumor_cna_file <- file.path(outdir, "GSE160269_P16T_infercna_CNA_tumor_only.rds")
...
cat("\n============================================================\n")
cat("13. P16T TUMOR VS NORMAL SUMMARY\n")
cat("============================================================\n\n")
...
fwrite(summary_dt, file.path(outdir, "GSE160269_P16T_18C_tumor_normal_summary.csv"), bom = TRUE)
fwrite(cell_metrics, file.path(outdir, "GSE160269_P16T_18C_cell_CNA_metrics.csv"), bom = TRUE)
...
fwrite(sample_summary, file.path(outdir, "GSE160269_P16T_18C_sample_QC.csv"), bom = TRUE)
...
fwrite(arm_dt, file.path(outdir, "GSE160269_P16T_18C_all_arm_metrics.csv"), bom = TRUE)
fwrite(coherent, file.path(outdir, "GSE160269_P16T_18C_candidate_coherent_arms.csv"), bom = TRUE)
...
heat_png <- file.path(figdir, "GSE160269_P16T_InferCNA_manual_all_cells_heatmap.png")
...
image(x = seq_len(ncol(z)), y = seq_len(nrow(z)), z = t(z), axes = FALSE, xlab = "Chromosome", ylab = "Cells", main = "GSE160269 P16T InferCNA Pilot (Limited Reference)", col = hcl.colors(101, "Blue-Red 3"))
...
cell_group[cell_order %in% tumor_cells] <- "P16T"
...
tumor_png <- file.path(figdir, "GSE160269_P16T_InferCNA_manual_tumor_only_heatmap.png")
...
image(x = seq_len(ncol(zt)), y = seq_len(nrow(zt)), z = t(zt), axes = FALSE, xlab = "Chromosome", ylab = "P16T Tumor Epithelial Cells", main = "P16T Tumor Epithelial CNA Profiles (Limited Reference)", col = hcl.colors(101, "Blue-Red 3"))
...
# ============================================================
# 20. SCATTER PLOT: CNA SIGNAL vs cnaCor
# ============================================================

cat("\n============================================================\n")
cat("17. SCATTER PLOT: CNA SIGNAL vs cnaCor\n")
cat("============================================================\n\n")

scatter_png <- file.path(figdir, "GSE160269_P16T_CNA_signal_vs_cnaCor_scatter.png")
...
plot(
  ...
  main = "P16T: CNA Signal vs cnaCor (Limited Reference)",
...
legend(
  "topleft",
  legend = c("Normal references (183 cells)", "Tumor (P16T)"),
  col = c("grey50", "red"),
  pch = 16,
  bty = "n",
  cex = 0.9
...
cat("\n============================================================\n")
cat("18. GSE160269 P16T PILOT REVIEW GATE\n")
cat("============================================================\n\n")
...
cat(sprintf("P16T tumor median CNA signal        : %.5f\n", tumor_signal_med))
...
cat(sprintf("Tumor > normal 99th CNA signal         : %.1f%%\n", 100 * mean(tumor_metrics$cna_signal > normal_signal_99, na.rm = TRUE)))
...
cat("\nNOTE: LIMITED REFERENCE (183 cells). Results are LOWER CONFIDENCE.\n")
...
manifest <- data.table(
  ...
  CNA_signal_delta = tumor_signal_med - normal_signal_med,
  CNA_signal_ratio = tumor_signal_med / normal_signal_med,
  correlation_separation = tumor_cor_med - normal_cor_med,
...
fwrite(manifest, file.path(outdir, "GSE160269_P16T_18C_manifest.csv"), bom = TRUE)

# ============================================================
# 23. RELOAD VALIDATION
# ============================================================

cat("\n===== 19. RELOAD CNA VALIDATION =====\n\n")

check <- readRDS(all_cna_file)
cat("Reloaded CDA: ", nrow(check), " genes x ", ncol(check), " cells\n", sep = "")
...
cat("\n============================================================\n")
cat("STEP 18C FINISHED\n")
cat("GSE160269 P16T INFERCNA PILOT COMPLETE\n")
cat("============================================================\n\n")

cat("Tumor epithelial cells      : ", length(tumor_cells), "\n", sep = "")
cat("Normal reference cells      : ", length(normal_cells), "\n", sep = "")
cat("Final CNA genes             : ", nrow(cna), "\n", sep = "")
cat("Tumor median CNA signal     : ", sprintf("%.5f", tumor_signal_med), "\n", sep = "")
cat("Normal median CNA signal    : ", sprintf("%.5f", normal_signal_med), "\n", sep = "")
cat("CNA signal ratio T/N        : ", sprintf("%.3f", tumor_signal_med / normal_signal_med), "\n", sep = "")
cat("Tumor median cnaCor         : ", sprintf("%.3f", tumor_cor_med), "\n", sep = "")
cat("Normal corr to tumor        : ", sprintf("%.3f", normal_cor_med), "\n", sep = "")
cat("Correlation separation      : ", sprintf("%.3f", tumor_cor_med - normal_cor_med), "\n", sep = "")
cat("Tumor > normal 95th         : ", sprintf("%.1f%%", 100 * mean(tumor_metrics$cna_signal > normal_signal_95, na.rm = TRUE)), "\n", sep = "")
cat("Tumor > normal 99th         : ", sprintf("%.1f%%", 100 * mean(tumor_metrics$cna_signal > normal_signal_99, na.rm = TRUE)), "\n", sep = "")
cat("Coherent CNA arms           : ", nrow(coherent), "\n", sep = "")
if (nrow(coherent) > 0) {
  cat("Coherent arm names          : ", paste(coherent$chromosome_arm, collapse = ", "), "\n", sep = "")
} else {
  cat("Coherent arm names          : none\n", sep = "")
}

cat("\nInferCNA run                : YES\n")
cat("findMalignant               : NO\n")
cat("Malignant labels assigned   : NO\n")
cat("Other samples processed     : NO\n")
